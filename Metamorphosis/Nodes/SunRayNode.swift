...
final class SunRayNode: SKShapeNode {
    ...
    init(topWidth: CGFloat, bottomWidth: CGFloat, height: CGFloat) {
        ...
        rayHeight = height
        bottomCurveDepth = GameConfig.sunRayBottomCurveDepth
        super.init()
        ...
        blurEffect.filter = CIFilter(
            name: "CIGaussianBlur",
            parameters: [kCIInputRadiusKey: GameConfig.sunRayEdgeBlurRadius]
        )
    ...
    func show() {
        isLightActive = true
        guard alpha < GameConfig.sunRayAlpha else { return }
        run(
            .fadeAlpha(
                to: GameConfig.sunRayAlpha,
                duration: GameConfig.sunRayTransitionDuration
            ),
            withKey: "SunRayFade"
        )
    }
    ...
    func hide() {
        isLightActive = false
        removeAction(forKey: "SunRayFade")
        run(.fadeOut(withDuration: 0.5), withKey: "SunRayFade")
    }
