...
final class DayNightController {
    ...
    init(scene: SKScene, windowPosition: CGPoint, windowSize: CGSize, sceneSize: CGSize) {
        ...
        nightDim.alpha = 0
        nightDim.name = "NightDim"

        sunRay = SunRayNode(
            topWidth: GameConfig.sunRayTopWidth,
            bottomWidth: GameConfig.sunRayBottomWidth,
            height: GameConfig.sunRayHeight
        )
        ...
        sunRay.setBottomOffset(GameConfig.sunRayBottomOffsets[0], animated: false)
    }
    ...
    func setSunRay(index: Int) {
        guard GameConfig.sunRayBottomOffsets.indices.contains(index) else { return }
        sunRay.setBottomOffset(GameConfig.sunRayBottomOffsets[index])
        sunRay.show()
    }
